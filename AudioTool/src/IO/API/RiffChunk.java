...
/**
 * General form of a RIFF chunk, the standard block of data contained within a
 * RIFF file.<br>
 * Also contains utilities for parsing and writing RIFF chunks, which may be
 * moved at a later date for neatness.<br>
 * <br>
 * <img src="RiffChunkParsing.png"/>
 * 
 * @author chuck
 *
 */
public abstract class RiffChunk implements Comparable<RiffChunk>
{
	...
	public RiffChunk getChildChunk(Class c)
	{
		System.out.println(this.getClass().getName() + ".getChildChunk(" + c.getName() + ") returned " + childMap.get(c).getClass().getName());
		return childMap.get(c);
	}

	/**
	 * Parse the given buffer and output all contained RIFF chunks, which may
	 * themselves contain RIFF chunks.
	 * 
	 * @param fileBuffer
	 *            The buffer containing the raw RIFF file bytes
	 * @param parentClass
	 *            The class of the RIFFChunk which contains the given byte
	 *            payload (at least as far as is specified by this chunks
	 *            'length' field)
	 * @return A list of all the child chunks found within the confines of the
	 *         chunk pointed to by this buffer, which may themselves contain
	 *         child chunks.
	 * @since Jul 14, 2012
	 */
	public static List<RiffChunk> ParseRiff(ByteBuffer fileBuffer, Class<? extends RiffChunk> parentClass)
	{
		...
		while (fileBuffer.position() < fileBuffer.capacity())
		{
			...
			tag = tag.replace(' ', '_');// Replace spaces in tags with
										// underscore
			// Convert tag sequence to String
			System.out.println("Looking for class to match tag: '" + tag + "'");
			final String className = packageName + ".RiffChunk_" + tag;
			try
			{
	...
	/**
	 * Parse the given buffer (minding its position) and build this RIFF chunk
	 * and field its child chunk hierarchy.<br>
	 * 
	 * @param fileBuffer
	 *            The buffer containing the raw RIFF data.
	 * @since Jul 15, 2012
	 */
	...
	/**
	 * Used for ensuring proper tag order when writing RIFF files because some
	 * utilities expect a specific tag order. (lower is earlier)
	 */
	@Override
	 ...
	 * Called internally when recursively building a chunk hierarchy from RIFF
	 * data.
	 * 
	 * @param buffer
	 * @since Jul 15, 2012
	...
	public final void toData(ByteBuffer buffer)
	{
		...
		try
		{
			if (!RIFFName.contentEquals("Root") && !className.contentEquals("Unrecognized"))
				buffer.put(RIFFName.getBytes());
		}
		catch (BufferOverflowException e)
		{
			throw new RuntimeException("Buffer Overflow Exception while pushing RIFF name into buffer of size " + buffer.capacity());
		}
		_toData(buffer);
	 ...
	 * Called internally when pre-calculating the size of a RIFF file. This
	 * portion is developer-specified.
	 * 
	 * @return
	 * @since Jul 15, 2012
	 */
	...
	/**
	 * Called externally and recursively when pre-calculating the size of a RIFF
	 * file.<br>
	 * 
	 * @return The cumulative size estimate of all child chunks and their
	 *         children, etc. plus the size of this chunk+4 for the RIFF nametag
	 *         (i.e. RIFF, WAVE, fmt, etc).
	 * @since Jul 15, 2012
	 */
	public final int sizeEstimateInBytes()
	{
		return childrenSizeEstimateInBytes() + _sizeEstimateInBytes() + 4;
	}// 4 is for the tag id
	...
	int childrenSizeEstimateInBytes()
	{
		...
		{
			accumulator += c.sizeEstimateInBytes();
		}
		return accumulator;
	}
	...
	{
		childMap.put(chunkToAdd.getClass(), chunkToAdd);
		System.out.println(this.getClass().getName() + ".addChildChunk(" + chunkToAdd.getClass().getName() + ")");
	}

	/**
	 * Prints the class names of all child chunks of this RiffChunk
	 * non-recursively.
	 * 
	 * @since Jul 15, 2012
	 */
	public void printChildChunks()
	{
		...
		if (childMap.isEmpty())
			System.out.println("	(No child chunks in this object)");
		for (RiffChunk c : childMap.values())
		{
